...
public class RootedTreeUtils {
	...
	public static boolean isUltrametric(RootedTree tree, double tolerance) {
		for (Node node : tree.getExternalNodesSet()) {
	...
	public static boolean isBinary(RootedTree tree) {
		for (Node node : tree.getInternalNodesSet()) {
	 ...
	 */
	public static Set<Node> getTipsForTaxa(RootedTree tree, Collection<Taxon> taxa) {
		...
		for (Taxon taxon : taxa) {
			...
			if (node == null) {
				throw new IllegalArgumentException(taxon.toString());
			}
	...
	public static boolean isMonophyletic(RootedTree tree, Set<Node> tipNodes) {
		...
		if (tipNodes.size() == tree.getExternalNodesSet().size()) {
	...
	public static String uniqueNewick(RootedTree tree, Node node) {
		if (tree.isExternal(node)) {
			return tree.getTaxon(node).toString();
		} else {
