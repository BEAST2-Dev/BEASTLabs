...
public class LogAnalyser extends Plugin {
    ...
    public Input<Boolean> m_DisplayAllStatistics =
            new Input<Boolean>("displayAllStatistics", "Display all availble atatistics result", false);
    ...
    public void initAndValidate() throws Exception {
        ...
        tracesReport(traces, m_DisplayAllStatistics.get());

        for (int i = 0; i < traces.getTraceCount(); i++) {
            TraceStatistics distribution = traces.analyseTrace(i);
            for (Expectation exp : m_pExpectations.get()) {
                if (traces.getTraceName(i).equals(exp.m_Name.get()))
                    analyseTrace(exp, distribution, m_DisplayAllStatistics.get(), null);
            }
        }
    ...
    public void analyseTrace(Expectation expectation, TraceStatistics distribution, boolean displayAllStatistics,
                             String likelihoodName) throws IOException, TraceException {

        double expectedValue = expectation.m_ExpValue.get();
        double mean = distribution.getMean();
        double error = distribution.getStdErrorOfMean();
        double ess = distribution.getESS();

        System.out.print(formatter.formatToFieldWidth(expectation.m_Name.get(), firstField));

        if (expectation.m_StandErrorOfMean.get() != 0) {
            error = expectation.m_StandErrorOfMean.get();
        }

        if (expectedValue > (mean - (2 * error)) && expectedValue < (mean + (2 * error))) {
            System.out.println("OK:       " + formatter.format(mean) + " +- " + formatter.format(error) + "\n");
        } else {
            System.out.print("WARNING: " + formatter.format(mean) + " +- " + formatter.format(error) + "\n");
        }
...
//            System.out.println(analysis.toString());
    ...
    private void tracesReport(LogFileTraces traces, boolean displayAllStatistics) {
        System.out.println();
        ...
        if (!displayAllStatistics)
            names = new String[]{"mean", "stdev", "hpdLower", "hpdUpper", "ESS"};
        else
            names = new String[]{"mean", "stdev", "hpdLower", "hpdUpper", "ESS", "stdErrorOfMeanFromLog",
