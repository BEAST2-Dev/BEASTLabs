...
public class FitchParsimony64 extends BEASTObject implements ParsimonyCriterion {
	...
	public void initAndValidate() {
		...
		this.union = new long[patternCount];
		this.intersection = new long[patternCount];
		
		
		states = new int[nodeCount][patternCount];
		stateSets = new long[nodeCount][patternCount];
	}
...
//			}
	...
	private void calculateSteps(Tree tree) {
		...
		for (int k = nodes.length - 1; k >= 0; --k) {
			...
			final long[] nodeStateSet = stateSets[node.getNr()];

			if (node.isLeaf()) {
				long[] stateSet = stateSets[node.getNr()];
				...
				for (Node child : node.getChildren()) {
					long[] childStateSet = stateSets[child.getNr()];
					if (first) {
	...
	private void reconstructStates(Node node, int [] parentStates) {
		...
		if (!node.isLeaf()) {
			long [] nodeStateSet = stateSets[node.getNr()];
			...
			for (int i = 0; i < patterns.getPatternCount(); i++) {
				...
				if (parentStates != null && (nodeStateSet[i] & parentStates[i]) > 0) {
					...
					int first = Long.numberOfTrailingZeros(nodeStateSet[i]);
					nodeStates[i] = first; 
				}
			}
